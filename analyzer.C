...
class Dijet{
	...
	public:
		Dijet();
		Dijet(TLorentzVector jet1, TLorentzVector jet2, TLorentzVector gen1, TLorentzVector gen2);
		TLorentzVector GetDijet() const;
		TLorentzVector GetJet1() const;
		TLorentzVector GetJet2() const;
		TLorentzVector GetGen1() const;
		TLorentzVector GetGen2() const;
		TLorentzVector GetDigen() const;
};
...
Dijet::Dijet(TLorentzVector jet1, TLorentzVector jet2, TLorentzVector gen1, TLorentzVector gen2):jet1_(jet1), jet2_(jet2), dijet_(jet1+jet2), gen1_(gen1), gen2_(gen2), digen_(gen1+gen2){
}

TLorentzVector Dijet::GetDijet() const {
	return dijet_;
}

TLorentzVector Dijet::GetJet1() const {
	return jet1_;
}

TLorentzVector Dijet::GetJet2() const {
	return jet2_;
}

TLorentzVector Dijet::GetGen1() const {
	return gen1_;
}

TLorentzVector Dijet::GetGen2() const {
	return gen2_;
}

TLorentzVector Dijet::GetDigen() const {
	return digen_;
}
		
bool wayToSort(const Dijet &dj1, const Dijet &dj2){
	return dj1.GetDijet().Pt() > dj2.GetDijet().Pt();
}
...
void analyzer (TString inputFileName,TString outputFileName, TString runPeriod, bool isData, bool isSignal, unsigned maxEvents, float SF)
{
  ...
  diJetOMass->Sumw2();
  TH1F* diGenOMass = new TH1F("diGenOMass","",50,0,200);
  ...
  near80->SetStats(1);
  near80->Sumw2(); 
  TH1F* GenWMass = new TH1F("GenWMass","",100,0,200);
  setHistTitles(GenWMass,"Mass[GeV/c^{2}","Evnets");
  GenWMass->SetStats(1);
  GenWMass->Sumw2();
  TH1F* GenM1 = new TH1F("GenM1","",10,-10,10);
  setHistTitles(GenM1,"Pt [GeV/c]","Events");
  GenM1->SetStats(1);
  GenM1->Sumw2();
  TH1F* GenM2 = new TH1F("GenM2","",10,-10,10);
  setHistTitles(GenM2,"Pt [GeV/c]","Events");
  GenM2->SetStats(1);
  GenM2->Sumw2();
  TH1F* ngenJet2 = new TH1F("ngenJet2","",6,0,6);
  setHistTitles(ngenJet2,"number of Jets","Events");
  ngenJet2->SetStats(1);
  ngenJet2->Sumw2();
  TH1F* genJet2n = new TH1F("genJet2n","",6,0,6);
...
// Order muons by pt
	...
	GenM2->Fill(reco2GenPostFSR.pt,weight);
    ...
    for(unsigned iJet=0; (iJet < unsigned(rawJets.nJets) && iJet < 10);iJet++)
    {
      if (rawJets.genPt[iJet]>0.0)
      {
        TLorentzVector tmpGenJetVec;
        tmpGenJetVec.SetPtEtaPhiM(rawJets.genPt[iJet],rawJets.genEta[iJet],rawJets.genPhi[iJet],rawJets.genMass[iJet]);
        genJets2.push_back(tmpGenJetVec);
      }
    }
...
//    /////////////////////////////////////////////
	...
	if(njets >= 2){
	  ...
	  std::sort(dijetvec.begin(),dijetvec.end(),wayToSort);
	  Dijet dijetMax;
	  int pairs = (njets*njets - njets)/2;
